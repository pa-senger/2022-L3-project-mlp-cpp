...
neuron::neuron(int n)
  ...
  for (int i = 0; i < n; ++i) {
    Weight_[i] = 1;   // the weights are set a 1 by default
    Weight_d_[i] = 0; // the weights derivatives are set a 0 by default
  }
}
...
void neuron::setWeightDerivative(double dWeight, int i) {
  if (i < 0) {
    std::cout << "Error : you must use positive integer for this !\n";
    exit(1);
  }
