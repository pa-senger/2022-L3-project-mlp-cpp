...
int neuron::unitTest1() { // tests constuctors
  ...
  if (n.size_X_ == 1 && n.post_activation_value_ == 0. &&
      n.pf_activation_ == &ReLU && n.pf_activation_d_ == &dReLU &&
      n.Weight_[0] == 1 && n.Weight_d_[0] == 0 && n.Weight_[1] == 0 &&
  ...
  // copy constructor
  if (n2.size_X_ == n1.size_X_ &&
      n2.post_activation_value_ == n1.post_activation_value_ &&
...
int neuron::unitTest2() { // tests getters setters
  ...
  if (n.getWeight(9) == 1.1 && n.getWeightDerivative(0) == 2.2 &&
...
int neuron::unitTest3() {
  ...
  ne1.setWeightsOnes();
  ne1 = ne2;
  if (ne2.size_X_ == ne1.size_X_ &&
      ne2.post_activation_value_ == ne1.post_activation_value_ &&
  ...
  for (int i = 0; i < ne1.size_X_ + 1; ++i) {
    if (ne2.Weight_[i] == ne1.Weight_[i] &&
        ne2.Weight_d_[i] == ne1.Weight_d_[i])
      ++k;
  }
  ...
  neu1.setBiais(1);
  neu2.setBiais(1);
  neu1.setActivationFcts(pf_s, pf_ds);
  neu2.setActivationFcts(pf_s, pf_ds);
  neu1.setWeight(4.1, 2);
  neu2.setWeight(4.1, 2);
  neu1.setWeightDerivative(3.1, 2);
  neu2.setWeightDerivative(3.1, 2);
  neu1.activateNeuron(X, 4);
  neu2.activateNeuron(X, 4);
  if ((neu1 == neu2) == true)
    ++passed;
  // passed = 11
  ...
  if ((neu1 != neu2) == true)
    ++passed;
  // passed = 13
  if (ne3.evaluateFct(5) == sigma(5) &&
      ne3.evaluateFctDerivative(1.1) == dSigma(1.1))
    ++passed;
  // passed = 14
